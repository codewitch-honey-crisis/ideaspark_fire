...
uint8_t fire_buffer[fire_height][fire_width]; // fire buffer, quarter resolution w/extra lines
...
// the lcd panel API display handle
...
#else
    esp_lcd_panel__disp_off(lcd_handle, false);
#endif
#ifdef LCD_BL
#ifdef LCD_BL > -1
...
void loop() {
    ...
    // call as another frame so we can check flush 
    // pending
